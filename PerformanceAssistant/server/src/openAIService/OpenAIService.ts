...
export class OpenAIService {
    ...
    try {
      ...
      const completionResponse = await openai.chat.completions.create({
        messages: [
          {
            role: "system",
            content: "Evaluate the candidate on the basis of data.",
          },
  ...
  public async insights(
    strengthData: CandidateAnalysisModel
  ): Promise<string | any> {
    const api_key = process.env.OAI_API_KEY;
    try {
      const openai = new OpenAI({ apiKey: api_key });
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an evaluator which evaluates and compares the strengths for each candidate on the basis of different parameters individually and with combined parameters and also provide role for each candidate",
          },
          { role: "user", content: prompt },
        ],
        model: "gpt-3.5-turbo",
  ...
  public convertToInsightModel(insightString: string): InsightModel {
    try {
      ...
      const cleanedInsightString = insightString
        .replace(/\\\"/g, '"')
        .replace(/\"/g, '"');
