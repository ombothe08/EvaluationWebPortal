...
export class Database {
  ...
  public async verifyUserCredentials(
    ...
    if (collection.length > 0 && Array.isArray(collection[0].users)) {
      ...
      for (const userData of users) {
        ...
        if (
          db_username === userCredentials.Email &&
          db_password === userCredentials.Password
        ) {
          return true;
        }
      }
  ...
  public async addReport(
    batchAnalysis: BatchAnalysisModel
  ): Promise<string | any> {
    if (!this.db) {
      throw new Error("Database connection is not established");
    }
  ...
  public async getReportById(
    reportId: string
  ): Promise<BatchReportDbModel | null> {
    if (!this.db) {
      throw new Error("Database connection is not established");
    }
      ...
      if (report) {
        ...
        const batchReportDbModel: BatchReportDbModel = {
          ...
          BatchData: {
            ...
            AnalysisModel: Array.isArray(report.BatchData.AnalysisModel)
              ? report.BatchData.AnalysisModel.map((data: any) => ({
                  Name: data.Name,
                  Strengths: Array.isArray(data.Strengths)
                    ...
                    : [],
                  AreasOfImprovement: Array.isArray(data.AreasOfImprovement)
                    ? data.AreasOfImprovement.map((improvement: any) => ({
                        Parameter: improvement.Parameter,
                        Data: improvement.Data,
                      }))
                    : [],
                  InputForMentors: Array.isArray(data.InputForMentors)
                    ? data.InputForMentors.map((input: any) => ({
  ...
  public async getAllRecords(
    ...
    try {
      ...
      if (records.length > 0) {
        const formattedRecords: BatchReportDbModel[] = records.map(
          (record) => ({
            objectid: record._id,
            BatchData: {
              ...
              AnalysisModel: Array.isArray(record.BatchData.AnalysisModel)
                ? record.BatchData.AnalysisModel.map((data: any) => ({
                    Name: data.Name,
                    Strengths: Array.isArray(data.Strengths)
                      ? data.Strengths.map((strength: any) => ({
                          Parameter: strength.Parameter,
        ...
        console.log(
          `Fetched ${records.length} records from ${collectionName} collection`
        );
  ...
  public async getInsightsByID(
    reportId: string
  ): Promise<BatchInsightDbModel | null> {
    if (!this.db) {
      throw new Error("Database connection is not established");
    }
    ...
    try {
      ...
      if (report) {
        ...
        const batchInsightDbModel: BatchInsightDbModel = {
          ...
          BatchData: {
            insight: {
              Data: Array.isArray(report.BatchData.insight.Data)
                ? report.BatchData.insight.Data.map((data: any) => ({
                    Name: data.Name,
                    CombineStrength: data.CombineStrength,
                    suggestedRole: data.suggestedRole,
                    insight: Array.isArray(data.insight)
                      ? data.insight.map((insight: any) => ({
